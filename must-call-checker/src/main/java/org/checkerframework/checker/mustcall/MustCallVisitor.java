...
public class MustCallVisitor extends BaseTypeVisitor<MustCallAnnotatedTypeFactory> {
  ...
  /**
   * Change the default for exception parameter lower bounds to bottom (the default), to prevent false
  ...
  /**
   * Annotation arguments are treated as return locations for the purposes of defaulting, rather than
   * parameter locations. This causes them to default incorrectly when the annotation is defined in bytecode.
   * See https://github.com/typetools/checker-framework/issues/3178 for an explanation of why this is
   * necessary to avoid false positives.
   *
   * <p>Skipping this check in the MustCall checker is safe, because the MustCall checker is not concerned with
   * annotation arguments (which must be literals, and therefore won't have must-call obligations).
   */
  @Override
