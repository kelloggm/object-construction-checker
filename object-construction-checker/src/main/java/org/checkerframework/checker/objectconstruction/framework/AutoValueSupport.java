...
public class AutoValueSupport implements FrameworkSupport{
	...
	public void handleConstructor(NewClassTree tree, AnnotatedTypeMirror type) {
	    ...
	    if (!superclass.getKind().equals(TypeKind.NONE)
	            && FrameworkSupportUtils.hasAnnotation(TypesUtils.getTypeElement(superclass), AutoValue.Builder.class)
...
//	        		elements.getAllannotationMirrors()
	    ...
	    if (FrameworkSupportUtils.isGuavaImmutableType(returnType) && allBuilderMethodNames.contains(name + "Builder")) {
	      return false;
	    }
	 ...
	 * @return list of all abstract methods
	      ...
	      for (Element member : t.getEnclosedElements()) {
	        ...
	        if (modifiers.contains(Modifier.ABSTRACT)) {
	          ...
	          overriddenMethods.addAll(
	              AnnotatedTypes.overriddenMethods(atypeFactory.getElementUtils(), atypeFactory, (ExecutableElement) member)
	                  .values());
	        }
	      }
	    }
	  ...
	  private List<Element> getAllSupertypes(Symbol symbol) {
	    Types types = Types.instance(((JavacProcessingEnvironment) atypeFactory.getProcessingEnv()).getContext());
